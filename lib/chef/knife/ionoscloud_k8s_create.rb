  ...
  class Knife
    class IonoscloudK8sCreate < Knife
      ...
      option :gateway_ip,
              ...
              description: 'The IP address of the gateway used by the cluster. This is mandatory when `public` is set to `false` and should '\
      ...
      def run
        ...
        cluster = Ionoscloud::KubernetesCluster.new(
          properties: Ionoscloud::KubernetesClusterPropertiesForPost.new(**cluster_properties),
        )

        cluster, _, headers = kubernetes_api.k8s_post_with_http_info(cluster)
