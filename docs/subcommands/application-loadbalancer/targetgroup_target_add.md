...
```text
knife ionoscloud targetgroup target add --extra-config EXTRA_CONFIG_FILE_PATH --target-group-id TARGET_GROUP_ID --ip IP --port PORT --weight WEIGHT --health-check-disable --maintenance --username USERNAME --password PASSWORD --url URL
```
